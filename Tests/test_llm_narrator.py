"""Tests fuer pb_studio.brain.llm_narrator (LM-Studio-Variante).
...
* LMStudioError beim /v1/models -> ``None`` (Fallback)
* NoSuitableModelError -> ``None``
* Post-Processing kuerzt auf max 3 Saetze und entfernt Praefixe/Bullets

Wir injecten einen ``LMStudioClient`` mit ``httpx.MockTransport`` damit kein
echter LM-Studio-Daemon laufen muss.
"""
...
def _make_handler(
    *,
    ...
    """Erzeugt einen MockTransport-Handler fuer /v1/models + /v1/chat/completions."""
    models_list = models or [DEFAULT_MODEL]

    def handler(request: httpx.Request) -> httpx.Response:
        path = request.url.path
        if path.endswith("/models") and request.method == "GET":
            if models_status >= 400:
                return httpx.Response(models_status, text="boom")
...
def test_generate_explanation_returns_text_from_model():
    ...
    client = _client(_make_handler(
        chat_content="Der Schnitt sitzt sauber auf dem Beat. Die Stimmung koennte dichter sein.",
    ))
...
def test_generate_explanation_uses_cache_on_second_call():
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if path.endswith("/models"):
            return httpx.Response(
                200,
                json={"data": [{"id": DEFAULT_MODEL, "object": "model"}], "object": "list"},
            )
        if path.endswith("/chat/completions"):
            call_counter["chat"] += 1
            return httpx.Response(
                200,
                json={
                    "id": "x", "object": "chat.completion", "model": DEFAULT_MODEL,
                    "choices": [{"index": 0,
                                 "message": {"role": "assistant", "content": "Der Cut wirkt stimmig."},
                                 "finish_reason": "stop"}],
                },
            )
        return httpx.Response(404)
        ...
        if path.endswith("/chat/completions"):
            ...
            return httpx.Response(
                ...
                json={
                    "id": "x", "object": "chat.completion", "model": DEFAULT_MODEL,
                    "choices": [{"index": 0,
                                 "message": {"role": "assistant", "content": f"Antwort {call_counter['chat']}."},
                                 "finish_reason": "stop"}],
                },
            )
        return httpx.Response(404)
...
# Error / Fallback
# ======================================================================
def test_generate_explanation_returns_none_on_lmstudio_unreachable():
    clear_narrative_cache()
    # models_status=500 -> Retries scheitern -> LMStudioConnectionError
    client = _client(_make_handler(models_status=500))
...
def test_generate_explanation_returns_none_when_no_model_installed():
    clear_narrative_cache()
    # Models-Endpoint liefert ein nicht-praeferiertes Modell
    client = _client(_make_handler(models=["totally-unrelated-model"]))
        ...
        if path.endswith("/chat/completions"):
            ...
            return httpx.Response(
                ...
                json={
                    "id": "x", "object": "chat.completion", "model": DEFAULT_MODEL,
                    "choices": [{"index": 0,
                                 "message": {"role": "assistant", "content": "Stimmig."},
                                 "finish_reason": "stop"}],
                },
            )
        return httpx.Response(404)
