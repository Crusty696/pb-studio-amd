...
def test_chat_content_is_not_copied_to_live_log_events(monkeypatch, caplog):
    ...
    async def run() -> str:
        await chat_router._history_store.clear()
        ...
        async for chunk in response.body_iterator:
            chunks.append(chunk)
        history = await chat_router._history_store.snapshot()
        assert history == [
...
def test_chat_exception_text_is_not_written_to_backend_logs(monkeypatch, caplog):
    from backend.routers import chat_router
    from pb_studio.ai import chat_agent
