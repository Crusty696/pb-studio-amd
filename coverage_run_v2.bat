...
python -m coverage run -c .coveragerc -m pytest -c pytest-coverage.ini Tests/ -q --tb=no 2>&1 >> coverage_v2_output.log
echo. >> coverage_v2_output.log
echo === Top 30 Coverage Report === >> coverage_v2_output.log
python -m coverage report --skip-empty --sort=-cover --fail-under=0 | head -40 >> coverage_v2_output.log 2>&1
echo. >> coverage_v2_output.log
...
python -m coverage report --skip-empty --sort=cover --fail-under=0 | head -25 >> coverage_v2_output.log 2>&1
