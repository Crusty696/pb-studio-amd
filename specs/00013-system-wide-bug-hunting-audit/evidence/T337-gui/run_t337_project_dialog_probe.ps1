...
$backendOut = Join-Path $PSScriptRoot 'backend-project-dialog.stdout.log'
$backendErr = Join-Path $PSScriptRoot 'backend-project-dialog.stderr.log'
...
try {
    ...
    & $pythonExe (Join-Path $PSScriptRoot 'inspect_project_dialog.py')
    if ($LASTEXITCODE -ne 0) {
        throw "Dialog probe exited with code $LASTEXITCODE."
    }
}
finally {
