...
- [X] T009 [OBJ-76] {(FR-396)} Versuche mit `scripts/diagnostics/verify_lmstudio_vlm.py`, `lms ps` und `lms log stream --source server --json` zuerst einen kalten sowie zwei warme Receipts des aktuell konfigurierten Captioning-Modells; bei bereits rotem offiziellen Load-Receipt entfallen Inferenzversuche. Prüfe qwen2.5-VL anschließend nur als dokumentierte Load-Kontrolle und stelle zuvor geladene fremde Modell-Identitäten danach gezielt wieder her
