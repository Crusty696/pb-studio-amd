    ...
    def _init_engine(self):
        try:
            ...
            if "DmlExecutionProvider" in available_providers:
                ...
                # KRITISCH: SessionOptions fuer DirectML patchen
                # audio-separator erstellt intern ort.SessionOptions() ohne
                # enable_mem_pattern=False - das crasht auf DirectML.
                # Monkey-Patch: Alle neuen SessionOptions bekommen automatisch
                # enable_mem_pattern=False wenn DirectML aktiv ist.
                _original_session_options_init = ort.SessionOptions.__init__
                def _patched_session_options_init(self_opts, *args, **kwargs):
                    _original_session_options_init(self_opts, *args, **kwargs)
                    self_opts.enable_mem_pattern = False
                ort.SessionOptions.__init__ = _patched_session_options_init
                logger.info("SessionOptions patched: enable_mem_pattern=False (DirectML)")
            else:
